...
export default class CodeHighlighterPlugin extends Plugin {
	...
	processCodeBlock(el: HTMLElement, ctx: MarkdownPostProcessorContext) {
		...
		codeBlocks.forEach((codeEl) => {
			...
			// Get the code fence info from the class attribute
			// Obsidian adds classes like "language-javascript" and may include our hl: syntax
			...
			const lines = ctx.getSectionInfo(preEl)?.text.split('\n') || [];
			...
			// Wrap each line in a span and apply highlighting
			const wrappedLines = codeLines.map((line, index) => {
				const lineNumber = index + 1;
				const shouldHighlight = this.shouldHighlightLine(lineNumber, line, rules);

				if (shouldHighlight) {
					return `<span class="code-highlighter-highlighted">${this.escapeHtml(line)}</span>`;
				}
